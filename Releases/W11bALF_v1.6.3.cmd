...
set fgy0=[m[38;2;032;032;032m
set fgy1=[m[38;2;064;064;064m
set fgy2=[m[38;2;092;092;092m
set fgy3=[m[38;2;128;128;128m
set fgy4=[m[38;2;192;192;192m
set fgy5=[m[38;2;255;255;255m
...
set winget_add=winget install %%winget_add1%% --silent --nowarn --accept-source-agreements --accept-package-agreements


:: Enfeites
set "line=echo:&echo %cin01%────────────────────────────────────────────────────────────%reset%&echo:"
...
if %errorlevel%==1 (goto elevate) else (goto chk_update)


:elevate	
echo    Quando solicitado %cin13% clique em %grn12% "SIM" %reset%

%powershell% "try { Start-Process -FilePath '%0' -ArgumentList 'am_admin' -Verb RunAs -ErrorAction Stop } catch { exit 1 }"
if %errorlevel%==1 goto not_admin

exit /b


:not_admin :: Elevação negada
cls
call :w11balf_wht
echo ░▒▓%cin13%            W11 but A Little Faster v%_ver%            %reset%▓▒░
echo ░▒▓%cin13%            %grn12%  github.com/ ru-bem/W11bALF  %cin13%            %reset%▓▒░
echo ░▒▓%cin13%                                                      %reset%▓▒░
echo %red11%     Não foi possível elevar os privilégios do programa     %reset%
%line%
echo    É necessário executar o programa como administrador para
echo    que todas os recursos funcionem corretamente.
%line%
echo    %grn11% SOLUÇÃO %reset%%grn01% Clique com o botão direito do mouse sobre o 
echo    W11bALF.cmd e escolha "Executar como administrador". %reset%
%line%
...
echo ░▒▓%cin13%  [ 0]  %reset%  Pacote Office %cin01%─────────────────────────────%reset%█▓▒░
...
%clwinsxs%
echo:
echo    Esse pode ser um processo lento mas pode ajudar a
echo    liberar muito espaço em armazenamento.
%line%
echo    %grn01%Se estiver executando isso em um %red01%HDD%grn01%, tenha paciência.%reset%
echo:
echo:
echo:
call :cmd_prog w22balf "dism.exe /online /Cleanup-Image /StartComponentCleanup /ResetBase"
echo:
echo [3A   %cin01%_%reset%
goto :eof
::
::
::
...
echo    %ui_inicio%        %cin01%^█%reset%  [8] Instalar codecs
echo                              %cin01%^█%reset%  [9] Abrir pasta Fontes
echo                              %cin01%^█%reset%  [A] Reiniciar o Explorer
echo    [6] Remover o Xbox App    %cin01%^█%reset%  [B] Gerenciar Widgets
echo    [7] Reinstalar o Xbox App %cin01%^█%reset%  
echo                              %cin01%^█%reset%
echo                              %cin01%^█%reset%
echo %cin01%▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄█▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄%reset%
...
echo    %progstep1% OpenCL e OpenGL
...
echo [9A   %progstep2% AV1
call :winget_add 9mvzqvxjbq9v
echo [1A   %progstep3% AV1%reset%

echo    %progstep2% HEIF
call :winget_add 9pmmsr1cgpwg
echo [1A   %progstep3% HEIF%reset%

echo    %progstep2% VP8 e VP9
call :winget_add 9n4d0msmp0pt
echo [1A   %progstep3% VP8 e VP9%reset%

echo    %progstep2% MPEG1 e MPEG2
call :winget_add 9n95q1zzpmh4
echo [1A   %progstep3% MPEG1 e MPEG2%reset%

echo    %progstep2% OpenCL e OpenGL
call :winget_add 9NQPSL29BFFF
echo [1A   %progstep3% OpenCL e OpenGL%reset%

echo    %progstep2% Web Experience Pack
call :winget_add 9n5tdp8vcmhs
...
echo [2A 
echo %grn12%& curl -# -L -o "%temp%\Microsoft.DesktopAppInstaller_8wekyb3d8bbwe.msixbundle" "https://github.com/microsoft/winget-cli/releases/download/v1.11.510/Microsoft.DesktopAppInstaller_8wekyb3d8bbwe.msixbundle"
echo %reset%
...
cls
call :w11balf_WHT
echo %grn12%             Instalar ou Desinstalar os Widgets             %reset%
%line%
echo %cin13%      %reset%
echo %cin13%    1 %reset%  Instalar os Widgets
echo %cin13%    2 %reset%  Desinstalar os Widgets
echo %cin13%      %reset%
%line%
echo %cin13%      %reset%
echo %cin13%    3 %reset%  Voltar aos extras
echo %cin13%    4 %reset%  Voltar ao menu principal
echo %cin13%      %reset%
%line%
set /p "widgets_c=----["

if %widgets_c%==1 goto widgets_add
if %widgets_c%==2 goto widgets_del
if %widgets_c%==3 goto extras
if %widgets_c%==4 goto w11menu
msg * /time:2 Esta não é uma opção válida. & goto widgets
...
echo %grn12%             Instalar ou Desinstalar os Widgets             %reset%
%line%
echo    Instalando os Widgets, aguarde...
%line%
call :winget_add 9MSSGKG348SP
call :winget_add 9N3RK8ZV2ZR8
echo    %grn01%Tudo pronto! %reset%
...
echo %grn12%             Instalar ou Desinstalar os Widgets             %reset%
%line%
echo    Desinstalando os Widgets, aguarde...
%line%
call :winget_del 9MSSGKG348SP
call :winget_del 9N3RK8ZV2ZR8
echo    %grn01%Tudo pronto! %reset%
echo    Pressione qualquer tecla para voltar.
pause >nul
goto widgets
...
:winget_del
winget uninstall %1 --force --silent --nowarn --accept-source-agreements >nul 2>&1
goto :eof
...
echo    %cin01% -  Padrão do mercado, mundialmente conhecido%reset%
echo:
echo:
...
if %office%==2 (curl -o "%temp%\W11bALF - O365.cmd" "https://raw.githubusercontent.com/ru-bem/W11bALF/refs/heads/main/Office/W11bALF%%20-%%20O365.cmd" >nul 2>&1& start "" "%temp%\W11bALF - O365.cmd"& goto w11menu)
...
echo:
echo:
set winget_add1=ONLYOFFICE.DesktopEditors
call :cmd_prog onlyoffice "%winget_add%"

:: call :winget_add ONLYOFFICE.DesktopEditors
...
:cmd_prog
start /min cmd /c "title %1 & %~2 & timeout /t 2 >nul & exit"
...
:cmd_exist
tasklist /v | find /i "%1" >nul 2>&1 && (goto:eof)
goto cmd_exist
...
:cmd_finished
echo [1A%grn01%   Processo concluído^^!                                   
echo    Pressione qualquer tecla para voltar
pause >nul
goto %3
...
:cmd_animation
set /a "n = (n + 1) %% 6"
if !n!==0 set "prog=%fgy5%─%fgy0%─%fgy1%─%fgy2%─%fgy3%─%fgy4%─"& echo [1A%prog%%prog%%prog%%prog%%prog%%prog%%prog%%prog%%prog%%prog%%reset%
if !n!==1 set "prog=%fgy4%─%fgy5%─%fgy0%─%fgy1%─%fgy2%─%fgy3%─"& echo [1A%prog%%prog%%prog%%prog%%prog%%prog%%prog%%prog%%prog%%prog%%reset%
if !n!==2 set "prog=%fgy3%─%fgy4%─%fgy5%─%fgy0%─%fgy1%─%fgy2%─"& echo [1A%prog%%prog%%prog%%prog%%prog%%prog%%prog%%prog%%prog%%prog%%reset%
if !n!==3 set "prog=%fgy2%─%fgy3%─%fgy4%─%fgy5%─%fgy0%─%fgy1%─"& echo [1A%prog%%prog%%prog%%prog%%prog%%prog%%prog%%prog%%prog%%prog%%reset%
if !n!==4 set "prog=%fgy1%─%fgy2%─%fgy3%─%fgy4%─%fgy5%─%fgy0%─"& echo [1A%prog%%prog%%prog%%prog%%prog%%prog%%prog%%prog%%prog%%prog%%reset%
if !n!==5 set "prog=%fgy0%─%fgy1%─%fgy2%─%fgy3%─%fgy4%─%fgy5%─"& echo [1A%prog%%prog%%prog%%prog%%prog%%prog%%prog%%prog%%prog%%prog%%reset%
goto:eof

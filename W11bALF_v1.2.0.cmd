...
reg query %reg_copilot% >nul 2>&1 & if %errorlevel%==1 reg add %reg_copilot% /t REG_DWORD /d 1 >nul 2>&1
...
echo ▓▒░                                                      ░▒▓
call :w11balf_WHT
echo %cin13%▓▒░                    Opções extras.                    ░▒▓
echo ▓▒░                                                      ░▒▓
echo ▓▒░                                                      ░▒▓%reset%
echo:
echo    %cin13% UTILIDADES (Todas as versões do Windows) %reset%
echo:
...
if %context%==0 (echo    [4] Ativar menu de contexto moderno) else (echo    [4] Ativar menu de contexto classico)
echo    [5] Reiniciar o Explorer
echo    [6] Configurar o MSEdge [BETA]
echo:
echo:
echo    %cin13% UTILIDADES (Windows LTSC) %reset%
echo:
echo    [7] Resetar/Instalar a Microsoft Store (Útil para LTSC)
echo    [8] Desinstalar a Microsoft Store %cin02%----%reset% (CUIDADO)
echo    [9] Resetar/Instalar o Winget %cin02%--------%reset% (Útil para LTSC)
echo:
echo:
%middleline%
echo:
...
if %extra%==9 goto winget

if %extra%==8 goto un_msstore

if %extra%==7 ver >nul & goto msstore

if %extra%==6 (call :medgecfg_info & %endmenu%)
...
if %extra%==4 (if %context%==0 (reg delete %reg_context% /f >nul 2>&1 & echo    Menu de contexto MODERNO ativado & taskkill /im explorer.exe /f >nul 2>&1 & for /l %%G in (1,1,4) do (timeout /t 1 >nul & tasklist | find /i "explorer.exe" >nul || (start explorer.exe >nul 2>&1 & goto extras) && goto extras)) else (reg add %reg_context% /f /ve >nul & echo    Menu de contexto CLASSICO ativado & taskkill /im explorer.exe /f >nul 2>&1 & for /l %%G in (1,1,4) do (timeout /t 1 >nul & tasklist | find /i "explorer.exe" >nul || (start explorer.exe >nul 2>&1 & goto extras) && goto extras)))
